...
fn create_and_list_puzzles() {
    ...
    let id = db.create_puzzle(env, &hash, "ipfs://clue").unwrap();
...
fn create_multiple_puzzles_different_envs() {
    ...
    db.create_puzzle("testnet", &h1, "ipfs://1").unwrap();
    db.create_puzzle("mainnet", &h2, "ipfs://2").unwrap();
...
fn mark_solved_valid_puzzle() {
    ...
    let id = db.create_puzzle(env, "0xaaa", "").unwrap();
...
fn add_and_update_delegation() {
    ...
    let pid = db.create_puzzle(env, "0xaaa", "").unwrap();
