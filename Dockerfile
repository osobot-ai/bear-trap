...
# Install r0vm in the builder stage (has rustc, which rzup requires).
# Version must match risc0-zkvm crate version (3.0.5).
# Copy to a stable path so COPY --from=builder doesn't depend on platform suffix.
...
# Copy r0vm from builder — needed by risc0-zkvm's default_executor() for preflight.
